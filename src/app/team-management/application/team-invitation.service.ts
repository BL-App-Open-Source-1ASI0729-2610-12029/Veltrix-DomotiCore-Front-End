...
export class TeamInvitationService {
  ...
  acceptInviteToken(token: string, user: InvitationUserContext | null | undefined): void {

    if (!token?.trim()) return;



    this.invitationsApi.acceptByToken(token.trim()).subscribe({
  ...
  private roleLabel(role: TeamMemberRole): string {
    const key = `teamManagement.roles.${role}`;
