...
export class TeamInvitationApiService {
  ...
  accept(invitationId: string): Observable<TeamInvitationRecord> {

    if (!this.api.hasApi()) {

      return of(this.readLocal().find(item => item.id === invitationId) ?? this.buildLocalRecord({
    ...
    return this.http
      ...
      .post<TeamInvitationApiResponse>(`${this.apiUrl('team-invitations')}/${invitationId}/accept`, {})

      .pipe(

        map(response => this.mapResponse(response)),

        tap(record => this.upsertLocal(record)),
  ...
  decline(invitationId: string): Observable<TeamInvitationRecord> {
    ...
    return this.http
      ...
      .post<TeamInvitationApiResponse>(`${this.apiUrl('team-invitations')}/${invitationId}/decline`, {})
  ...
  resend(invitationId: string): Observable<TeamInvitationRecord> {
    ...
    if (!this.api.hasApi()) {
      ...
      const existing = this.readLocal().find(item => item.id === invitationId);

      return of(existing ?? this.buildLocalRecord({
    ...
    return this.http
      ...
      .post<TeamInvitationApiResponse>(`${this.apiUrl('team-invitations')}/${invitationId}/resend`, {})
  ...
  acceptByToken(token: string): Observable<TeamInvitationRecord> {

    if (!this.api.hasApi()) {

      return of(this.buildLocalRecord({
    ...
    return this.http
      ...
      .post<TeamInvitationApiResponse>(this.apiUrl('team-invitations/accept-by-token'), { token })
